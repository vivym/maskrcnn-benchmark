...
class DeformRoIPoolingPack(DeformRoIPooling):
    ...
    def forward(self, data, rois):
        ...
        if len(rois) <= 0:
            return torch.empty((0, input.size(1), self.pooled_size, self.pooled_size)).to(input.device)
...
class ModulatedDeformRoIPoolingPack(DeformRoIPooling):
    ...
    def forward(self, data, rois):
        ...
        if len(rois) <= 0:
            return torch.empty((0, data.size(1), self.out_size, self.out_size)).to(data.device)
